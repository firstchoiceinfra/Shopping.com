...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS riders (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        username TEXT UNIQUE,
        password TEXT,
        full_name TEXT,
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS rider_earnings (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        rider_id INTEGER,
        order_id INTEGER,
        amount REAL,
    ...
    # ---- Safe migration: add columns to tables that may already exist from an older run ----
    def safe_add_column(table, coldef):
    ...
    for coldef in ["rider_id INTEGER", "cancel_reason TEXT", "delivered_at TEXT"]:
        safe_add_column("orders", coldef)
    ...
    if cur.fetchone()[0] == 0:
        cur.execute('''INSERT INTO riders (username, password, full_name, phone, city, is_online, total_earnings, created_at)
            VALUES (?,?,?,?,?,?,?,?)''',
            ("rider_amit", "rider123", "Amit Kumar", "9900011122", "Mumbai", 1, 0.0,
...
# ============================================================
ORDER_STATUSES = ["Placed", "Confirmed", "Packed", "Out for Delivery", "Delivered", "Cancelled"]
STATUS_COLOR = {
    "Placed": "badge-gold", "Confirmed": "badge-gold", "Packed": "badge-gold",
    "Out for Delivery": "badge-gold", "Delivered": "badge-green", "Cancelled": "badge-red"
}

def advance_order_status(order_id, new_status, rider_id=None):
    conn = get_conn()
    cur = conn.cursor()
    if new_status == "Delivered":
        cur.execute("UPDATE orders SET status=?, delivered_at=? WHERE id=?",
                    (new_status, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), order_id))
        if rider_id:
            row = cur.execute("SELECT delivery_charge FROM orders WHERE id=?", (order_id,)).fetchone()
            fee = row[0] if row else 0.0
            cur.execute("UPDATE riders SET total_earnings = total_earnings + ? WHERE id=?", (fee, rider_id))
            cur.execute("INSERT INTO rider_earnings (rider_id, order_id, amount, timestamp) VALUES (?,?,?,?)",
                        (rider_id, order_id, fee, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    elif new_status == "Out for Delivery" and rider_id:
        cur.execute("UPDATE orders SET status=?, rider_id=? WHERE id=?", (new_status, rider_id, order_id))
    else:
...
def cancel_order(order_id):
    """Refund the commission back to the shopkeeper wallet and mark order cancelled."""
    conn = get_conn()
    cur = conn.cursor()
    row = cur.execute("SELECT shopkeeper_id, commission_amount, status FROM orders WHERE id=?", (order_id,)).fetchone()
    if not row:
        conn.close()
        return False
    shopkeeper_id, commission, status = row
    if status in ("Delivered", "Cancelled", "Out for Delivery"):
        conn.close()
        return False
    bal_row = cur.execute("SELECT wallet_balance FROM shopkeepers WHERE id=?", (shopkeeper_id,)).fetchone()
    new_bal = (bal_row[0] if bal_row else 0) + commission
    cur.execute("UPDATE shopkeepers SET wallet_balance=?, is_active=1 WHERE id=?", (new_bal, shopkeeper_id))
    cur.execute('''INSERT INTO wallet_transactions (shopkeeper_id, txn_type, amount, balance_after, note, timestamp)
        VALUES (?,?,?,?,?,?)''', (shopkeeper_id, "credit", commission, new_bal, "Commission refund (order cancelled)",
                                   datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
...
def build_invoice_text(order_row):
    items = json.loads(order_row['items'])
    lines = [
        "========= LocalBazaar Invoice =========",
        f"Order ID: {order_row['id']}",
        f"Date: {order_row['timestamp']}",
        f"Shop: {order_row['shop_name']}",
        f"Customer: {order_row['customer_name']}",
        f"Status: {order_row['status']}",
        "-" * 40,
    ]
    for it in items:
        lines.append(f"{it['name']:<25} x{it['qty']:<3} ₹{it['price']:.2f}")
    lines += [
        "-" * 40,
        f"Subtotal:        ₹{order_row['subtotal']:.2f}",
        f"Delivery Charge: ₹{order_row['delivery_charge']:.2f}",
        f"GST (18%):       ₹{order_row['gst_amount']:.2f}",
...
def do_login(role, username, password):
    ...
    if role == "Admin":
        ...
        row = cur.execute("SELECT id FROM riders WHERE username=? AND password=?", (username, password)).fetchone()
        if row:
            st.session_state.update(logged_in=True, role="Delivery Partner", current_user=username, rider_id=row[0])
            conn.close()
            return True
    conn.close()
    return False
...
if not st.session_state.logged_in:
    ...
    role_choice = st.sidebar.selectbox("Main hoon:", ["Customer", "Shopkeeper", "Delivery Partner", "Admin"])
    ...
    if role_choice == "Admin":
        ...
        with tabs_login:
            ...
            if role_choice == "Delivery Partner":
                st.caption("Demo rider: rider_amit / rider123 · ya naya register karein.")
        ...
        with tabs_register:
            ...
            elif role_choice == "Delivery Partner":
                st.markdown("**Delivery Partner ban kar kamaayein**")
                ru = st.text_input("Username", key="r_rider_user")
                rp = st.text_input("Password", type="password", key="r_rider_pass")
                rn = st.text_input("Aapka naam", key="r_rider_name")
                rph = st.text_input("Phone", key="r_rider_phone")
                rcity = st.text_input("City", key="r_rider_city")
                if st.button("Register as Rider", key="reg_rider_submit"):
                    if ru and rp:
                        conn = get_conn()
                        try:
                            conn.execute('''INSERT INTO riders (username, password, full_name, phone, city, is_online, total_earnings, created_at)
                                VALUES (?,?,?,?,?,1,0,?)''', (ru, rp, rn, rph, rcity, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                            conn.commit()
                            st.success("Rider account ban gaya! Ab login karein aur orders deliver karke kamaayein.")
                        except sqlite3.IntegrityError:
                            st.error("Yeh username pehle se hai.")
                        conn.close()
                    else:
    ...
    if st.sidebar.button("Logout"):
        for k in ["logged_in", "role", "current_user", "shopkeeper_id", "rider_id"]:
...
if st.session_state.role == "Customer":
    nav_options = ["🏠 Nearby Shops (Search)", "🛒 Cart & Checkout", "📦 My Orders", "❤️ Wishlist", "👤 My Profile"]
elif st.session_state.role == "Shopkeeper":
    nav_options = ["📊 Shop Dashboard", "💰 Wallet & Commission", "🛠️ Manage Products", "📦 Orders Received"]
elif st.session_state.role == "Delivery Partner":
    nav_options = ["🛵 Available Orders", "📦 My Deliveries", "💰 My Earnings"]
else:  # Admin
    nav_options = ["📊 Platform Overview", "🏪 All Shopkeepers", "🛵 All Riders", "📦 All Orders", "🛡️ Audit Logs", "📥 Reports & Export"]
    ...
    if selected_option == "🏠 Nearby Shops (Search)":
        ...
        query = '''
            SELECT p.*, s.shop_name, s.area, s.city, s.lat as shop_lat, s.lon as shop_lon,
                   ...
                   (SELECT AVG(rating) FROM reviews r WHERE r.shopkeeper_id = s.id) as shop_rating
            FROM products p
            JOIN shopkeepers s ON p.shopkeeper_id = s.id
            WHERE s.is_active = 1 AND p.stock > 0
        '''
        df = pd.read_sql_query(query, conn)
        wishlist_ids = pd.read_sql_query("SELECT product_id FROM wishlist WHERE customer_name=?",
                                          conn, params=(st.session_state.current_user,))['product_id'].tolist()
        conn.close()

        f1, f2, f3, f4 = st.columns([2, 1, 1, 1])
        search_q = f1.text_input("🔍 Kya chahiye? (e.g. rice, mouse, notebook, oil...)")
        cat_options = ["Sabhi"] + sorted([c for c in df['category'].dropna().unique().tolist() if c])
        cat_filter = f2.selectbox("Category", cat_options)
        max_price = int(df['price'].max()) if not df.empty else 1000
        price_range = f3.slider("Price ≤", 0, max(max_price, 100), max(max_price, 100))
        sort_by = f4.selectbox("Sort by", ["Nearest", "Price: Low to High", "Price: High to Low"])
        ...
        if sort_by == "Nearest" and st.session_state.customer_lat:
        ...
        if df.empty:
            st.info("Koi product nahi mila. Search/filter badal kar dekhein.")
        else:
            st.caption(f"{len(df)} products mile.")
            cols = st.columns(3)
            for i, row in df.reset_index(drop=True).iterrows():
                with cols[i % 3]:
                    ...
                    rating_txt = f"⭐ {row['shop_rating']:.1f}" if pd.notnull(row['shop_rating']) else "⭐ New"
                    st.markdown(f"""
                        <div class="premium-card">
                            ...
                            <div style="margin-top:8px;font-size:12px;color:#64748B;">
                                🏪 {row['shop_name']} ({row['area']}, {row['city']}) &nbsp; {rating_txt}<br/>
                                📍 {dist_txt} &nbsp;|&nbsp; ⏱️ {eta}
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
                    bc1, bc2 = st.columns([3, 1])
                    if bc1.button("➕ Add to Cart", key=f"add_{row['id']}"):
                        st.session_state.cart[int(row['id'])] = st.session_state.cart.get(int(row['id']), 0) + 1
                        st.success("Cart mein add ho gaya!")
                    is_wished = int(row['id']) in wishlist_ids
                    if bc2.button("💔" if is_wished else "❤️", key=f"wish_{row['id']}"):
                        conn = get_conn()
                        if is_wished:
                            conn.execute("DELETE FROM wishlist WHERE customer_name=? AND product_id=?",
                                        (st.session_state.current_user, int(row['id'])))
                        else:
                            conn.execute("INSERT INTO wishlist (customer_name, product_id, timestamp) VALUES (?,?,?)",
                                        (st.session_state.current_user, int(row['id']), datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                        conn.commit()
                        conn.close()
                        st.rerun()
        ...
        reviewed_ids = pd.read_sql_query("SELECT order_id FROM reviews WHERE customer_name=?",
                                         conn, params=(st.session_state.current_user,))['order_id'].tolist()
        conn.close()
        if odf.empty:
            st.info("Abhi tak koi order nahi hai.")
        else:
            for _, o in odf.iterrows():
                badge = STATUS_COLOR.get(o['status'], "badge-gold")
                track = " → ".join([f"**{s}**" if s == o['status'] else s
                                    for s in ["Placed", "Confirmed", "Packed", "Out for Delivery", "Delivered"]])
                st.markdown(f"""
                    <div class="premium-card">
                        <b>🏪 {o['shop_name']}</b> &nbsp; <span class="badge {badge}">{o['status']}</span><br/>
                ...
                if o['status'] != "Cancelled":
                    st.caption(f"📍 Tracking: {track}")

                bcol1, bcol2, bcol3 = st.columns([1, 1, 2])
                invoice_txt = build_invoice_text(o)
                bcol1.download_button("🧾 Invoice", data=invoice_txt, file_name=f"invoice_order_{o['id']}.txt",
                                      key=f"inv_{o['id']}")
                if o['status'] in ("Placed", "Confirmed"):
                    if bcol2.button("❌ Cancel Order", key=f"cancel_{o['id']}"):
                        if cancel_order(int(o['id'])):
                            st.success("Order cancel ho gaya aur shop ka commission refund ho gaya.")
                            st.rerun()
                if o['status'] == "Delivered" and o['id'] not in reviewed_ids:
                    with bcol3.popover("⭐ Rate this order"):
                        rt = st.slider("Rating", 1, 5, 5, key=f"rt_{o['id']}")
                        cmt = st.text_area("Comment", key=f"cmt_{o['id']}")
                        if st.button("Submit Review", key=f"submit_rev_{o['id']}"):
                            conn = get_conn()
                            conn.execute('''INSERT INTO reviews (order_id, shopkeeper_id, customer_name, rating, comment, timestamp)
                                VALUES (?,?,?,?,?,?)''', (int(o['id']), int(o['shopkeeper_id']), st.session_state.current_user,
                                                          rt, cmt, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                            conn.commit()
                            conn.close()
                            st.success("Review submit ho gaya, dhanyavaad!")
                            st.rerun()

    elif selected_option == "❤️ Wishlist":
        st.subheader("❤️ Meri Wishlist")
        conn = get_conn()
        wdf = pd.read_sql_query('''
            SELECT p.*, s.shop_name FROM wishlist w
        ...
        conn.close()
        if wdf.empty:
            st.info("Wishlist khali hai. 'Nearby Shops' page se ❤️ dabakar products add karein.")
        else:
            cols = st.columns(3)
            for i, row in wdf.reset_index(drop=True).iterrows():
                with cols[i % 3]:
                    st.markdown(f"""
                        <div class="premium-card">
                            <img src="{row['image_url']}" style="width:100%;height:130px;object-fit:cover;border-radius:10px;" onerror="this.style.display='none'"/>
                            <h4 style="margin:8px 0 2px 0;">{row['name']}</h4>
                            <div style="font-size:12px;color:#64748B;">🏪 {row['shop_name']}</div>
                            <span class="price-tag">₹{row['price']:,.0f}</span>
                        </div>
                    """, unsafe_allow_html=True)
                    wc1, wc2 = st.columns(2)
                    if wc1.button("➕ Cart", key=f"wl_add_{row['id']}"):
                        st.session_state.cart[int(row['id'])] = st.session_state.cart.get(int(row['id']), 0) + 1
                        st.success("Added!")
                    if wc2.button("🗑️ Remove", key=f"wl_del_{row['id']}"):
                        conn = get_conn()
                        conn.execute("DELETE FROM wishlist WHERE customer_name=? AND product_id=?",
                                    (st.session_state.current_user, int(row['id'])))
                        conn.commit()
...
if st.session_state.role == "Shopkeeper":
    ...
    if selected_option == "📊 Shop Dashboard":
        ...
        st.markdown("---")
        conn = get_conn()
        low_stock = pd.read_sql_query("SELECT name, stock FROM products WHERE shopkeeper_id=? AND stock < 5", conn, params=(sk_id,))
        conn.close()
        if not low_stock.empty:
            st.warning("⚠️ **Low Stock Alert** — yeh products jaldi khatam ho rahe hain:")
        ...
        if odf.empty:
            ...
            for _, o in odf.iterrows():
                ...
                st.markdown(f"""
                    <div class="premium-card">
                        <b>Order #{o['id']} from {o['customer_name']}</b> &nbsp; <span class="badge {badge}">{o['status']}</span><br/>
                ...
                """, unsafe_allow_html=True)
                if o['status'] == "Placed":
                    if st.button("✅ Confirm Order", key=f"conf_{o['id']}"):
                        advance_order_status(int(o['id']), "Confirmed")
                        st.rerun()
                elif o['status'] == "Confirmed":
                    if st.button("📦 Mark Packed", key=f"pack_{o['id']}"):
                        advance_order_status(int(o['id']), "Packed")
                        st.rerun()
                elif o['status'] == "Packed":
                    conn = get_conn()
                    riders_df = pd.read_sql_query("SELECT id, full_name, username FROM riders WHERE is_online=1", conn)
                    conn.close()
                    if riders_df.empty:
                        st.warning("Koi delivery partner online nahi hai abhi.")
                    else:
                        rider_pick = st.selectbox("Delivery Partner assign karein", riders_df['id'],
                                                   format_func=lambda x: riders_df[riders_df['id'] == x]['full_name'].iloc[0] or riders_df[riders_df['id'] == x]['username'].iloc[0],
                                                   key=f"riderpick_{o['id']}")
                        if st.button("🛵 Send for Delivery", key=f"send_{o['id']}"):
                            advance_order_status(int(o['id']), "Out for Delivery", rider_id=int(rider_pick))
                            st.rerun()

# ============================================================
# DELIVERY PARTNER (RIDER) PAGES
# ============================================================
if st.session_state.role == "Delivery Partner":
    rider_id = st.session_state.rider_id
    conn = get_conn()
    rider = pd.read_sql_query("SELECT * FROM riders WHERE id=?", conn, params=(rider_id,)).iloc[0]
    conn.close()

    if selected_option == "🛵 Available Orders":
        st.subheader("🛵 Delivery ke liye taiyaar orders")
        online = st.toggle("🟢 Main online hoon (orders milenge)", value=bool(rider['is_online']))
        if online != bool(rider['is_online']):
            conn = get_conn()
            conn.execute("UPDATE riders SET is_online=? WHERE id=?", (1 if online else 0, rider_id))
            conn.commit()
            conn.close()
            st.rerun()
        ...
        avail = pd.read_sql_query('''
            SELECT o.*, s.area, s.city FROM orders o
            JOIN shopkeepers s ON o.shopkeeper_id = s.id
            WHERE o.status='Packed' AND (o.rider_id IS NULL OR o.rider_id=0)
            ORDER BY o.id DESC
        ''', conn)
        conn.close()
        if not online:
            st.info("Offline hain — pehle online toggle karein orders dekhne ke liye.")
        elif avail.empty:
            st.info("Abhi koi order packed nahi hai jo delivery ke liye ready ho.")
        else:
            for _, o in avail.iterrows():
                st.markdown(f"""
                    <div class="premium-card">
                        <b>Order #{o['id']}</b> — {o['shop_name']} ({o['area']}, {o['city']})<br/>
                        <span style="font-size:12px;color:#64748B;">Distance: {o['distance_km']:.1f} km</span><br/>
                        💰 Delivery earning: <b>₹{o['delivery_charge']:,.2f}</b>
                    </div>
                """, unsafe_allow_html=True)
                if st.button("🛵 Pick this delivery", key=f"pick_{o['id']}"):
                    advance_order_status(int(o['id']), "Out for Delivery", rider_id=rider_id)
                    st.success("Order aapko assign ho gaya! 'My Deliveries' mein dekhein.")
                    st.rerun()

    elif selected_option == "📦 My Deliveries":
        st.subheader("📦 Meri Deliveries")
        conn = get_conn()
        mine = pd.read_sql_query("SELECT * FROM orders WHERE rider_id=? ORDER BY id DESC", conn, params=(rider_id,))
        conn.close()
        if mine.empty:
            st.info("Abhi koi delivery assign nahi hai.")
        else:
            for _, o in mine.iterrows():
                badge = STATUS_COLOR.get(o['status'], "badge-gold")
                st.markdown(f"""
                    <div class="premium-card">
                        <b>Order #{o['id']}</b> — {o['shop_name']} &nbsp; <span class="badge {badge}">{o['status']}</span><br/>
                        <span style="font-size:12px;color:#64748B;">Customer: {o['customer_name']} · {o['distance_km']:.1f} km</span><br/>
                        💰 Aap kamaayenge: <b>₹{o['delivery_charge']:,.2f}</b>
                    </div>
                """, unsafe_allow_html=True)
                if o['status'] == "Out for Delivery":
                    if st.button("✅ Mark Delivered", key=f"deliv_{o['id']}"):
                        advance_order_status(int(o['id']), "Delivered", rider_id=rider_id)
                        st.success("Delivered! Earning wallet mein add ho gayi.")
                        st.rerun()

    elif selected_option == "💰 My Earnings":
        st.subheader("💰 Meri Kamai")
        conn = get_conn()
        latest = pd.read_sql_query("SELECT total_earnings FROM riders WHERE id=?", conn, params=(rider_id,))['total_earnings'].iloc[0]
        edf = pd.read_sql_query("SELECT * FROM rider_earnings WHERE rider_id=? ORDER BY id DESC", conn, params=(rider_id,))
        conn.close()
        st.metric("Total Lifetime Earnings", f"₹{latest:,.2f}")
        if not edf.empty:
            fig = px.bar(edf.sort_values('id'), x='id', y='amount', title="Delivery-wise Earnings",
                        color='amount', color_continuous_scale=['#FDE68A', '#F59E0B'])
            st.plotly_chart(fig, use_container_width=True)
            st.dataframe(edf, use_container_width=True, hide_index=True)
        else:
            st.info("Abhi koi earning nahi hai — deliveries complete karein.")
...
if st.session_state.role == "Admin":
    ...
    if selected_option == "📊 Platform Overview":
    ...
    elif selected_option == "🛵 All Riders":
        st.subheader("🛵 All Delivery Partners")
        rdf = pd.read_sql_query("SELECT id, username, full_name, phone, city, is_online, total_earnings FROM riders", conn)
        ...
        with pd.ExcelWriter(out, engine='openpyxl') as writer:
            ...
            pd.read_sql_query("SELECT * FROM riders", conn).to_excel(writer, sheet_name='Riders', index=False)
            pd.read_sql_query("SELECT * FROM reviews", conn).to_excel(writer, sheet_name='Reviews', index=False)
